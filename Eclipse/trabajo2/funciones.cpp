	...
	Mat binaria;
	    ...
	    for (int i = 0; i < puntosHarrys.rows - tamanoVentana; i++) {
	        for (int j = 0; j < puntosHarrys.cols - tamanoVentana; j++) {
	            ...
	            if (esMax == true) {
	                binaria.at<unsigned char> (i + (tamanoVentana / 2) + 1, j + (tamanoVentana / 2) + 1) = 1;
	            }
	    ...
	    for (int i = 0; i < binaria.rows; i++) {
	        for (int j = 0; j < binaria.cols; j++) {
	            if (binaria.at<unsigned char>(i, j) == 1) {
	                resultado.push_back(PointH(Point(j, i), nivel, puntosHarrys.at<double>(i, j)));
	            }
	        }
	    }
 ...
 * CAMBIAR!!
 */
vector<PointH> listPointHarris(Mat img)
{
	...
	for (unsigned int k = 0; k < pyramid.size(); k++)
	{
		...
		for (int i = 0; i < eigenValuesH.rows; i++)
		{
			...
			{
				eigenValuesH.at<double>(i, j) = HarrisPixel(eigenValuesH.at<Vec3d>(i, j));
			}
		}
...
{
    vector<PointH> point_harris = listPointHarris(img);

    drawCircles(img, point_harris);
